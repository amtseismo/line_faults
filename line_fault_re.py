...
#    o['Lb']=o['G']*o['dc']/(o['sigma']*o['b']);
#    o['hstar']=o['b']/(o['b']-o['a'])*o['Lb']
...
#    Arguments:
#        t :  time
#        y :  vector of the state variables [velocity,theta,slip,stress]
...
#
## Call the ODE solver.
#wsol = solve_ivp(lambda t, y: ratestate(t, y, o), [0,300], y0, max_step=100, rtol=10e-10, atol=10e-10)
#
## Make plots
## Plot the result
...
x = wsol.t
y = o['X'][o['Wstart']:o['Wend']]
X, Y = np.meshgrid(x, y)
state = wsol.y[o['Wstart']:o['Wend'],:]
cs = ax.pcolormesh(Y, X, state,cmap='jet')
cbar = fig.colorbar(cs)
plt.ylabel('Time (s)')
plt.xlabel('State')

fig, ax = plt.subplots(figsize=(10,15))
vel = wsol.y[o['Wstart']+o['N']:o['Wend']+o['N'],:]
