...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 11:49:21 2019

Dimensional line fault obeying rate and state friction
...
def ratestate_sl(t,y,o):
    ...
    dVeldt = np.concatenate((np.zeros(o['Fstart']),
        ...
        (-o['bgrid'][o['Fstart']:o['Fend']]*dStatedt[o['Fstart']:o['Fend']]/State[o['Fstart']:o['Fend']] - o['G']/o['sigma']*stress[o['Fstart']:o['Fend']]/2),
        np.zeros(o['N']-o['Fend'])))
    dDispdt = Vel
...
# Call the ODE solver.
if o['sl']:
    wsol = solve_ivp(lambda t, y: ratestate_sl(t, y, o), [0,1e8], y0, max_step=1000000, rtol=10e-10, atol=10e-10)
else:
    wsol = solve_ivp(lambda t, y: ratestate_al(t, y, o), [0,1e8], y0, max_step=1000000, rtol=10e-10, atol=10e-10)
